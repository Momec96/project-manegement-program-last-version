...
def groups():
    ...
    tablouzunluk=len(team1)
    if len(team2)>len(team1):
        tablouzunluk=len(team2)
    # Add a Treeview widget
    tree = ttk.Treeview(win, column=("Takım 1 üyeleri", "Takım 2 üyeleri"), show='headings', height=tablouzunluk)
    tree.column("# 1", anchor=CENTER)
    tree.heading("# 1", text="Takım 1 üyeleri")
    tree.column("# 2", anchor=CENTER)
    tree.heading("# 2", text="Takım 2 üyeleri")

    if len(team1)<tablouzunluk:
        team1kisileri.append("-")
        cur.execute("INSERT INTO team1 (isim) VALUES (?)",("-"))
    if len(team2)<tablouzunluk:
        team2kisileri.append("-")
        cur.execute("INSERT INTO team2 (isim) VALUES (?)",("-"))
    conn.commit()
